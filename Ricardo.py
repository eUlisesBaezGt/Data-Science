...
class PlotWin:
    def window(self):
        ...
        self.lbl4 = tk.Label(self.window, text = "Rf = ")
        self.lbl4.place(x = 10, y = 130)

        self.txt1 = tk.Entry(self.window, width=35)
        self.txt1.place(x = 50, y = 10)
        prSetTxt(self.txt1, "sin(cos(x*.33))", "cos(sin(y*.55))")

        self.sbLog = tk.Scrollbar(self.window)
        self.sbLog.pack(side = tk.RIGHT, fill = tk.Y)

        self.txtLog = tk.Text(self.window, bg = "Purple", width = 60, height = 5, wrap = tk.NONE,
                            yscrollcommand = self.sbLog.set)
